...
def cadastro():
    ...
    if request.method == 'POST':
        colunas_map = {
            'MUNICIPIO': 'municipio', 'DISTRITO': 'distrito', 'EMPRESA': 'empresa', 'CNPJ': 'cnpj',
            'PROCESSO SEI': 'processo_sei', 'STATUS DE ASSENTAMENTO': 'status_de_assentamento',
            'RAMO DE ATIVIDADE': 'ramo_de_atividade', 'EMPREGOS GERADOS': 'empregos_gerados',
            'QUADRA': 'quadra', 'MÓDULO(S)': 'modulo_s', 'QTD. MÓDULOS': 'qtd_modulos',
            'TAMANHO(M²)': 'tamanho_m2', 'MATRÍCULA(S)': 'matricula_s', 'OBSEVAÇÕES': 'obsevacoes',
            'DATA ESCRITURAÇÃO': 'data_escrituracao', 'DATA CONTRATO DE COMPRA E VENDA': 'data_contrato_de_compra_e_venda',
            'IRREGULARIDADES?': 'irregularidades', 'ÚLTIMA VISTORIA': 'ultima_vistoria',
            'ATUALIZADO': 'atualizado', 'IMÓVEL REGULAR/IRREGULAR': 'imovel_regular_irregular',
            ...
            with mysql.connector.connect(**db_config) as db:
                with db.cursor() as cursor:
                    ...
                    if file and file.filename.strip():
                        ...
                        file.save(filepath)
                        caminho_imagem = f"/static/imagens_empresas/{nome_arquivo}"
                        cursor.execute("INSERT INTO empresa_infos (empresa_id, descricao, caminho_imagem) VALUES (%s, %s, %s)", 
                                     (empresa_id, request.form.get('DESCRICAO_EMPRESA', ' '), caminho_imagem))
                        db.commit()

                    gravar_log(f"CADASTRO_EMPRESA (ID {empresa_id})", db_conn=db)
...
def selecionar_edicao():
    try:
        with mysql.connector.connect(**db_config) as db:
            with db.cursor(dictionary=True) as cursor:
                cursor.execute("SELECT id, municipio, empresa, cnpj FROM municipal_lots WHERE empresa != '-' ORDER BY empresa")
        ...
        with mysql.connector.connect(**db_config) as db:
            with db.cursor(dictionary=True) as cursor:
                if request.method == 'POST':
                    ...
                    valores = [request.form.get(col, '') for col in campos] + [empresa_id]
                    cursor.execute(f"UPDATE municipal_lots SET {set_clause} WHERE id = %s", valores)
                    db.commit()
                    gravar_log(f"EDIÇÃO_ASSENTAMENTO (ID {empresa_id})", db_conn=db)
        ...
        with mysql.connector.connect(**db_config) as db:
            with db.cursor(dictionary=True) as cursor:
                if request.method == 'POST':
                    ...
                    cursor.execute(f"UPDATE municipal_lots SET {set_clause} WHERE id = %s", valores)
                    db.commit()
                    gravar_log(f"EDIÇÃO_JURIDICA (ID {empresa_id})", db_conn=db)
...
# --- ROTA DE RELATÓRIOS --- (Mantida conforme original)
...
def relatorios():
    if request.method == 'POST':
        ...
        try:
            ...
            template = PageTemplate(id='watermark_template', frames=[frame], onPage=add_watermark)
            ...
            story = [NextPageTemplate('watermark_template')]
            logo_path = os.path.join(app.root_path, 'static', 'logo_codego.png')
            if os.path.exists(logo_path):
                story.append(Image(logo_path, width=200, height=40, hAlign='CENTER'))
            
            story.append(Paragraph(f"RELATÓRIO: {lot.get('empresa', 'N/A')}", title_style))
            
            data = [["Campo", "Valor"]]
            for k, v in lot.items():
                data.append([Paragraph(str(k).upper(), cell_style), Paragraph(str(v) if v else '-', cell_style)])

            table = Table(data, colWidths=[150, 350])
            table.setStyle(TableStyle([('GRID', (0,0), (-1,-1), 0.5, colors.grey), ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#1a233a')), ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke)]))
            story.append(table)
    ...
    try:
        with mysql.connector.connect(**db_config) as db:
            with db.cursor(dictionary=True) as cursor:
                ...
                for row in infos:
                    empresas_info[str(row['empresa_id'])] = {"descricao": row['descricao'], "foto": row['caminho_imagem']}
    except: pass
...
def registrar_usuario():
    if request.method == 'POST':
        nome, login_user, email, senha, depto = request.form.get('nome'), request.form.get('login'), request.form.get('email'), request.form.get('senha'), request.form.get('departamento', '')
        if not all([nome, login_user, email, senha]):
        ...
        try:
            with mysql.connector.connect(**db_config) as db:
                with db.cursor() as cursor:
                    cursor.execute("INSERT INTO usuarios (nome, login, email, senha, departamento) VALUES (%s, %s, %s, %s, %s)",
                                 (nome, login_user, email, bcrypt.generate_password_hash(senha).decode('utf-8'), depto))
                    db.commit()
